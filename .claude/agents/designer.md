...
3. Read the bundled `ux-research` skill (`.claude/skills/ux-research/`) and ground the design behaviorally BEFORE any visual decision: who the user is + the job-to-be-done + context of use (from the spec, not invented), 3-5 named psychology principles for this surface type, the design-time usability checklist, and the ethics hard-lines (no dark patterns — violating them means the design spec is rejected).
4. If the `ui-ux-pro-max` skill is available, use it to generate a design system + a11y checklist. Take what fits, override what doesn't match the brand (the skill is a multiplier, not the source of truth). If it's unavailable, run these principles manually. Then consult the bundled `premium-design` skill (`.claude/skills/premium-design/`) — the quality bar this design spec must meet: palette discipline, typography metrics, motion with a purpose, and context adaptation (bold for marketing, restraint for product/dashboard). The brand wins on any conflict.
5. Write the design spec to `.pipeline/design.md`:
   - **User & principles** — who + JTBD + context of use, then **Principles used**: each psychology principle mapped to where it's applied (per the `ux-research` skill).
...
6. If any asset needs prep (logo to transparent, image optimization), do it via Bash if possible — or write precise instructions for the Coder. Don't make the Coder guess.
